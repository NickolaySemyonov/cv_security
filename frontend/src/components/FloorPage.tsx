...
const FloorPage = ({ user, onLogout }: FloorPageProps) => {
  ...
  const handleEdit = () => {
    console.log('Редактирование этажа:', currentFloor?.id);
    // TODO: открыть модальное окно для редактирования карты
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex-grow">
        <div className="bg-white rounded-2xl shadow-md p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            onClick={handleEdit}
            className="bg-yellow-500 text-white px-5 py-2 rounded-xl hover:bg-yellow-600 transition-all duration-200 flex items-center gap-2"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
